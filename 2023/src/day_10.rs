...
pub fn solve(input: &str) -> Solution {
...

    {
        ...
        for tile in Tile::PIPES {
            ...
            {
                *grid.get_signed_mut(start_column, start_row).unwrap() = tile;
            }
        }
    }
    ...
    while let Some((row, column, current_distance)) = vec.pop_front() {
            ...
            .flat_map(|directions| directions.iter())
        {
            ...
            if let Some(visited) = visited_grid.get_signed(column, row).copied() {
                if !visited {
                    *visited_grid.get_signed_mut(column, row).unwrap() = true;
...
impl Tile {
    ...
    fn connects(&self) -> Option<[Direction; 2]> {
        match self {
            Tile::Ground => None,
            Tile::NorthSouth => Some([Direction::North, Direction::South]),
            Tile::EastWest => Some([Direction::East, Direction::West]),
            Tile::NorthEast => Some([Direction::North, Direction::East]),
            Tile::NorthWest => Some([Direction::North, Direction::West]),
            Tile::SouthEast => Some([Direction::South, Direction::East]),
            Tile::SouthWest => Some([Direction::South, Direction::West]),
        }
    }
}
