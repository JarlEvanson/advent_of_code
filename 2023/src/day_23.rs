...
pub fn solve(input: &str) -> Solution {
    ...
    for (y, row) in (0..grid.height()).map(|row| (row as isize, grid.row(row).unwrap())) {
        ...
        {
            let neighbors: (&[Direction], &[Direction]) = match tile {
                '#' => continue,
                '.' => (&Direction::ALL, &Direction::ALL),
                '^' => (core::slice::from_ref(&Direction::North), &Direction::ALL),
                '>' => (core::slice::from_ref(&Direction::East), &Direction::ALL),
                'v' => (core::slice::from_ref(&Direction::South), &Direction::ALL),
                '<' => (core::slice::from_ref(&Direction::West), &Direction::ALL),
                _ => unreachable!(),
            };
