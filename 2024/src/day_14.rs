...
pub fn solve(input: &str) -> (usize, usize) {
    ...
    for pos in robots.clone().map(|(start, velocity)| {
        (
            calc_position(start.0, velocity.0, WIDTH, 100),
            calc_position(start.1, velocity.1, HEIGHT, 100),
        )
    }) {
        if pos.0 < middle_x {
            if pos.1 < middle_y {
    ...
    let part_1 = upper_right * upper_left * lower_right * lower_left;

    let mut part_2 = 0;
    while part_2 < 100_000 {
